...
**Auth architecture: password-first, with OIDC.** Login now defaults to password (a real user database, not passwordless-only); magic-code-by-email remains available as a fallback step. OIDC login (authorization code + PKCE, JWKS-verified ID tokens) is built and provider-agnostic — tested against a mock IdP, not yet configured against the real Pocket ID instance at id.majid.film (needs real client_id/secret/redirect URI registered there; `OIDC_*` vars documented, commented-out, in `.env.example`). A verified-email match against an existing account auto-links; no match auto-creates a user — deliberate for a single-tenant, owner-controlled instance where the provider's own login (passkey-backed on Pocket ID) is treated as sufficient identity signal. Passkeys themselves are entirely Pocket ID's concern — no native WebAuthn in Review.
